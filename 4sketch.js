...
let sidebarItems = [];
let linkElements = []; // HTML <a> elements

// MASTER RESOURCE OBJECT
// =========================
let resources = {
...
"Movie/TV Download Sites": [
  ...
  "Giveaways (your new list) combine this with general giveaways": [
...
"Overlays (your new list) combine with overlay giveaways": [
...
"TikTok Growth Tutorials": [
  ["(Growth) How to Grow on TikTok", "https://youtu.be/placeholder233"],
  ["(Growth) TikTok Algorithm Explained", "https://youtu.be/placeholder234"],
  ["(Growth) Best Posting Times", "https://youtu.be/placeholder235"],
  ["(Growth) How to Go Viral", "https://youtu.be/placeholder236"],
  ["(Growth) Editing for Growth", "https://youtu.be/placeholder237"]
],
...
function drawMainPanel() {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    link.position(sidebarWidth + 20, y + 24 - mainScroll);
...
function mouseWheel(event) {
  if (mouseX < sidebarWidth) {
    ...
    let links = resources[selectedCategory];
    let contentHeight = links.length * 60 + 120;
...
function updateLinkPositions() {
  ...
  for (let i = 0; i < linkElements.length; i++) {
    linkElements[i].position(sidebarWidth + 20, y + 24 - mainScroll);
    y += 60;
  }
}
